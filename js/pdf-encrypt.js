// pdf-encrypt.js — Binary-Safe PDF Encryption (RC4 128-bit)
// Bekerja langsung pada Uint8Array untuk mencegah PDF Blank.
...
// MD5 Implementation (Binary-Safe)
function md5(input) {
  const bytes = input;
...
function padPassword(pw) {
  ...
  result.set(pwBytes);
  if (pwBytes.length < 32) result.set(PDF_PAD.slice(0, 32 - pwBytes.length), pwBytes.length);
  return result;
}
...
async function encryptPDF(pdfBytes, userPassword, ownerPassword) {
  ...
  const perm = -3904; // Standard permissions
  ...
  let oValue = rc4(ownerKey, padPassword(userPw));
  for (let i = 1; i <= 19; i++) {
    const xorKey = ownerKey.map(b => b ^ i);
  ...
  const encryptionKeySeed = concatBytes(paddedUser, oValue, pBytes, fileId);
  let encKey = md5(encryptionKeySeed);
  for (let i = 0; i < 50; i++) encKey = md5(encKey);
  encKey = encKey.slice(0, 16);

  // 3. Compute U Value
  const uSeed = md5(concatBytes(PDF_PAD, fileId));
  let uValue = rc4(encKey, uSeed);
  for (let i = 1; i <= 19; i++) {
    const xorKey = encKey.map(b => b ^ i);
    uValue = rc4(xorKey, uValue);
  }

  // 4. Encrypt PDF Objects (Binary Scanning)
  // Kita harus membagi PDF menjadi bagian-bagian objek
  const pdfStr = bytesToStr(pdfBytes); // Menggunakan latin1 agar aman untuk biner
  const objRegex = /(\d+)\s+(\d+)\s+obj([\s\S]*?)stream([\r\n]+)([\s\S]*?)endstream/g;
  
  let resultStr = pdfStr;
  let offset = 0;
  
  // Ganti stream dalam objek dengan versi terenkripsi
  resultStr = pdfStr.replace(objRegex, (match, objNum, genNum, dict, eol, stream, offset) => {
    // Jangan enkripsi XRef stream
    if (dict.includes('/XRef')) return match;

    const oNum = parseInt(objNum);
    const gNum = parseInt(genNum);
    
    // Create Object Key
    const extra = new Uint8Array([oNum & 0xff, (oNum>>8) & 0xff, (oNum>>16) & 0xff, gNum & 0xff, (gNum>>8) & 0xff]);
    const objKey = md5(concatBytes(encKey, extra)).slice(0, Math.min(encKey.length + 5, 16));
    
    const encryptedStream = rc4(objKey, new Uint8Array(Array.from(stream).map(c => c.charCodeAt(0))));
    return `${objNum} ${genNum} obj${dict}stream${eol}${bytesToStr(encryptedStream)}endstream`;
  });
  ...
  resultStr = resultStr.replace(/trailer\s*<<([\s\S]*?)>>/g, (m, inner) => {
